...
class ProfilePreferencesStorage(
    ...
    override suspend fun getLastProfile(): ProfileSnapshot? = withContext(Dispatchers.IO) {
        val lastId = prefs.getString(KEY_LAST_PROFILE_ID, null)
        if (lastId.isNullOrBlank()) return@withContext null
        val raw = prefs.getString(profileCacheKey(lastId), null)
        if (raw.isNullOrBlank()) return@withContext null
        runCatching { json.decodeFromString(ProfileSnapshot.serializer(), raw) }
            .getOrNull()
    }
