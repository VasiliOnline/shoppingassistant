...
class TrackFilterOptionsRepositoryImpl(
    ...
    private data class ExtraFacetDescriptor(
        val displayKey: String,
        val facetKey: String,
        val normalizedDisplayKey: String,
        val normalizedFacetKey: String,
    )
    ...
    private suspend fun loadFacetCounts(
        ...
        return when (key) {
            TrackFilterKey.EXTRA_VALUE -> {
                val facetKey = extraKey?.let { mapExtraKeyToFacetKey(it, extraDescriptors) } ?: return emptyMap()
            ...
            TrackFilterKey.EXTRA_KEY -> {
                ...
                val narrowedByQuery = if (query.isNullOrBlank()) {
                    ...
                    descriptors.filter { descriptor ->
                        descriptor.displayKey.lowercase().contains(query) ||
                            descriptor.facetKey.lowercase().contains(query)
                    }
                }
                val descriptorsForCounts = narrowedByQuery.take(MAX_EXTRA_KEYS_FOR_COUNTS)
                val counts = LinkedHashMap<String, Int>()
                descriptorsForCounts.forEach { descriptor ->
                    val map = loadFacetCountsForFacet(resolvedCategory, descriptor.facetKey, query)
                    val total = map.values.sum()
                    if (total > 0) {
                        val existing = counts[descriptor.displayKey] ?: 0
                        counts[descriptor.displayKey] = existing + total
                    }
                }
                counts
            }
    ...
    private fun seededOptionsFor(
        ...
        return when (key) {
            ...
            TrackFilterKey.EXTRA_KEY -> extraDescriptors.map { it.displayKey }
            TrackFilterKey.EXTRA_VALUE -> {
    ...
    private fun buildExtraFacetDescriptors(
        ...
        val descriptors = orderedCodes
            .mapNotNull { code ->
                val def = defsByCode[code] ?: return@mapNotNull null
                val displayKey = def.title.trim().takeIf { it.isNotBlank() } ?: def.code.trim()
                val facetKey = def.code.trim()
                if (displayKey.isBlank() || facetKey.isBlank()) return@mapNotNull null
                ExtraFacetDescriptor(
                    displayKey = displayKey,
                    facetKey = facetKey,
                    normalizedDisplayKey = normalizeToken(displayKey),
                    normalizedFacetKey = normalizeToken(facetKey),
                )
            }

        return descriptors
            .distinctBy { descriptor -> descriptor.normalizedFacetKey }
            .sortedBy { descriptor -> descriptor.displayKey.lowercase() }
    }

    private fun mapExtraKeyToFacetKey(
        extraKey: String,
        descriptors: List<ExtraFacetDescriptor>,
    ): String? {
        val normalized = normalizeToken(extraKey)
        if (normalized.isEmpty()) return null
        descriptors.firstOrNull { descriptor ->
            descriptor.normalizedDisplayKey == normalized ||
                descriptor.normalizedFacetKey == normalized
        }?.let { descriptor ->
            return descriptor.facetKey
        }
    ...
    private companion object {
        ...
        const val MAX_EXTRA_KEYS_FOR_COUNTS = 30
        val MULTI_SPACES_REGEX = Regex("\\s+")
