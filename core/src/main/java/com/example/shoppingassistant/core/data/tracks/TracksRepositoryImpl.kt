...
class TracksRepositoryImpl(
    ...
    override suspend fun createTrack(input: TrackCreateInput): Track {
        if (canUseRemote()) {
            val request = TrackCreateRequest(
                type = input.type,
                target = TrackTarget(matchKey = input.matchKey),
        ...
        val localTrack = Track(
            id = generatedId,
            title = input.title ?: input.matchKey ?: input.categoryCode ?: "Отслеживание",
            categoryCode = input.categoryCode,
            type = input.type,
            target = TrackTarget(
                matchKey = input.matchKey,
                query = input.categoryCode.takeIf { input.type == TrackType.CATEGORY },
            ),
            filters = input.filters,
    ...
    override suspend fun updateTrackTarget(
        ...
        val nextTrack = current.copy(
            ...
            filters = current.filters.copy(extra = normalizeAttributes(input.attributes)),
        )
