...
class Stage21AutoPackageLoaderTest {

    @Test
    fun coverage_required_leaves_are_reachable_via_browse_tree() {
        val coverage = Stage21AutoPackageLoader.coverageGate
        val browseByCode = Stage21AutoPackageLoader.browseNodes.associateBy { it.browseCode }

        val missingLeafBrowsePaths = coverage.requiredLeafCodes.filter { leafCode ->
            Stage21AutoPackageLoader.browseNodes
                .asSequence()
    ...
    fun coverage_minimums_met_for_aliases_and_golden_queries() {
        ...
        val browseLeafByCode = Stage21AutoPackageLoader.browseNodes
            .asSequence()
            .mapNotNull { node ->
        ...
        val aliasesByLeaf = Stage21AutoPackageLoader.aliasSeedRows
            .asSequence()
        ...
        val goldenByLeaf = Stage21AutoPackageLoader.goldenQueries
            .asSequence()
    ...
    fun blocked_alias_share_is_within_coverage_limit() {
        val coverage = Stage21AutoPackageLoader.coverageGate
        val rows = Stage21AutoPackageLoader.aliasSeedRows
