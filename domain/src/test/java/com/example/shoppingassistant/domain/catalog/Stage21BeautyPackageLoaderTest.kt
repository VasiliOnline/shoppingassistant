...
class Stage21BeautyPackageLoaderTest {

    @Test
    fun coverage_required_leaves_are_reachable_via_browse_tree() {
        val coverage = Stage21BeautyPackageLoader.coverageGate
        val browseByCode = Stage21BeautyPackageLoader.browseNodes.associateBy { it.browseCode }

        val missingLeafBrowsePaths = coverage.requiredLeafCodes.filter { leafCode ->
            Stage21BeautyPackageLoader.browseNodes
                .asSequence()
    ...
    fun coverage_minimums_met_for_aliases_and_golden_queries() {
        ...
        val browseLeafByCode = Stage21BeautyPackageLoader.browseNodes
            .asSequence()
            .mapNotNull { node ->
        ...
        val aliasesByLeaf = Stage21BeautyPackageLoader.aliasSeedRows
            .asSequence()
        ...
        val goldenByLeaf = Stage21BeautyPackageLoader.goldenQueries
            .asSequence()
    ...
    fun blocked_alias_share_is_within_coverage_limit() {
        val coverage = Stage21BeautyPackageLoader.coverageGate
        val rows = Stage21BeautyPackageLoader.aliasSeedRows
