...
class Stage21FoodPackageLoaderTest {

    @Test
    fun coverage_required_leaves_are_reachable_via_browse_tree() {
        val coverage = Stage21FoodPackageLoader.coverageGate
        val browseByCode = Stage21FoodPackageLoader.browseNodes.associateBy { it.browseCode }

        val missingLeafBrowsePaths = coverage.requiredLeafCodes.filter { leafCode ->
            Stage21FoodPackageLoader.browseNodes
                .asSequence()
    ...
    fun coverage_minimums_met_for_aliases_and_golden_queries() {
        ...
        val browseLeafByCode = Stage21FoodPackageLoader.browseNodes
            .asSequence()
            .mapNotNull { node ->
        ...
        val aliasesByLeaf = Stage21FoodPackageLoader.aliasSeedRows
            .asSequence()
        ...
        val goldenByLeaf = Stage21FoodPackageLoader.goldenQueries
            .asSequence()
    ...
    fun blocked_alias_share_is_within_coverage_limit() {
        val coverage = Stage21FoodPackageLoader.coverageGate
        val rows = Stage21FoodPackageLoader.aliasSeedRows
