...
class Stage21HomePackageLoaderTest {

    @Test
    fun coverage_required_leaves_are_reachable_via_browse_tree() {
        val coverage = Stage21HomePackageLoader.coverageGate
        val browseByCode = Stage21HomePackageLoader.browseNodes.associateBy { it.browseCode }

        val missingLeafBrowsePaths = coverage.requiredLeafCodes.filter { leafCode ->
            Stage21HomePackageLoader.browseNodes
                .asSequence()
    ...
    fun coverage_minimums_met_for_aliases_and_golden_queries() {
        ...
        val browseLeafByCode = Stage21HomePackageLoader.browseNodes
            .asSequence()
            .mapNotNull { node ->
        ...
        val aliasesByLeaf = Stage21HomePackageLoader.aliasSeedRows
            .asSequence()
        ...
        val goldenByLeaf = Stage21HomePackageLoader.goldenQueries
            .asSequence()
    ...
    fun blocked_alias_share_is_within_coverage_limit() {
        val coverage = Stage21HomePackageLoader.coverageGate
        val rows = Stage21HomePackageLoader.aliasSeedRows
