...
class Stage21SportPackageLoaderTest {

    @Test
    fun coverage_required_leaves_are_reachable_via_browse_tree() {
        val coverage = Stage21SportPackageLoader.coverageGate
        val browseByCode = Stage21SportPackageLoader.browseNodes.associateBy { it.browseCode }

        val missingLeafBrowsePaths = coverage.requiredLeafCodes.filter { leafCode ->
            Stage21SportPackageLoader.browseNodes
                .asSequence()
    ...
    fun coverage_minimums_met_for_aliases_and_golden_queries() {
        ...
        val browseLeafByCode = Stage21SportPackageLoader.browseNodes
            .asSequence()
            .mapNotNull { node ->
        ...
        val aliasesByLeaf = Stage21SportPackageLoader.aliasSeedRows
            .asSequence()
        ...
        val goldenByLeaf = Stage21SportPackageLoader.goldenQueries
            .asSequence()
    ...
    fun blocked_alias_share_is_within_coverage_limit() {
        val coverage = Stage21SportPackageLoader.coverageGate
        val rows = Stage21SportPackageLoader.aliasSeedRows
