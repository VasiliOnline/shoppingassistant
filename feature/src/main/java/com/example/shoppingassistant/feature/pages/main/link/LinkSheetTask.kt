...
fun LinkSheet(
    ...
    val screenState = when (state) {
        is LinkSheetState.Input -> ScreenState.Content()
        is LinkSheetState.Loading -> ScreenState.Loading(LoadingPhase.INITIAL)
        is LinkSheetState.Error -> ScreenState.Error(
            message = state.message,
            primaryAction = StateAction(
                label = "Попробовать другую ссылку",
                onAction = onRetry,
                type = StateActionType.CHANGE_QUERY,
            ),
        )
    }
    val loadingContent: (@Composable (PaddingValues) -> Unit)? = when (state) {
        is LinkSheetState.Loading -> ({ _ -> LinkSheetLoading(state) })
        else -> null
    }
    ...
    Box(
        ...
        Surface(
            ...
            StateHost(
                ...
                screenName = "link_sheet",
            ) { innerPadding ->
                Box(modifier = Modifier.padding(bottom = innerPadding.calculateBottomPadding())) {
                    if (state is LinkSheetState.Input) {
                        LinkSheetInput(
                            input = input,
...
private fun LinkSheetInput(
    ...
    Column(
        ...
        Text(
            text = "Скопируйте и вставьте ссылку на предложение с площадки",
...
@Composable
private fun LinkSheetLoading(state: LinkSheetState.Loading) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
        ...
        Row(
            ...
            Text(
                text = "Загружаем и разбираем ссылку…",
                style = MaterialTheme.typography.titleMedium,
            )
        }
        LazyColumn(
