...
class TrackedOfferRepositoryImpl(
    ...
    override suspend fun createTrackedOffer(request: TrackedOfferInput): CreateTrackedOfferResult =
        DatabaseFactory.dbQuery {
            ...
            val productId = ProductsTable.insert { stmt ->
                ...
                stmt[ProductsTable.specs] = if (attrs.isNotEmpty()) attrs else null
            ...
            val offerId = OffersTable.insert { stmt ->
                ...
                stmt[OffersTable.attributes] = if (attrs.isNotEmpty()) attrs else null
    ...
    override suspend fun refreshTrackedOffer(request: RefreshTrackedOfferInput): RefreshTrackedOfferResult =
        DatabaseFactory.dbQuery {
            ...
            val currency = request.currency.trim()
            if (price.isNaN() || price <= 0 || currency.length != 3) {
                return@dbQuery RefreshTrackedOfferResult(
                    status = RefreshTrackedOfferStatus.INVALID_INPUT,
            ...
            val newAttrs = request.attributes.filterValues { it.isNotBlank() }
            val currentAttrs = existing[OffersTable.attributes]
            val merged = if (newAttrs.isNotEmpty()) {
                (currentAttrs ?: emptyMap()) + newAttrs
            } else {
                currentAttrs
            }
            val condition = normalizeCondition(merged?.get("condition"))
                ?: existing[OffersTable.condition]
            val deliveryChannel = normalizeDeliveryChannel(
                merged?.get("delivery_channel") ?: merged?.get("delivery")
            ) ?: existing[OffersTable.deliveryChannel]

            val priceCents = toMinorUnits(price)
            OffersTable.update({ OffersTable.id eq offerId }) { stmt ->
                stmt[OffersTable.priceCents] = priceCents
                stmt[OffersTable.currency] = currency.uppercase()
                stmt[OffersTable.attributes] = merged
                stmt[OffersTable.condition] = condition
                stmt[OffersTable.deliveryChannel] = deliveryChannel
                stmt[OffersTable.updatedAt] = System.currentTimeMillis()
            }

            OfferPriceHistoryTable.insert { stmt ->
                stmt[OfferPriceHistoryTable.offerId] = offerId
                stmt[OfferPriceHistoryTable.priceMinor] = priceCents
                stmt[OfferPriceHistoryTable.currency] = currency.uppercase()
    ...
    private fun validate(request: TrackedOfferInput): String? {
        val title = request.title.trim()
        if (title.length !in 3..160) return "Title length must be 3..160"
        if (request.category.name.isBlank()) return "Category is required"
        val brand = request.brand?.trim()
        if (brand != null && brand.length > 120) return "Brand is too long"
        val model = request.model
        if (model != null && model.length > 160) return "Model is too long"
        val price = request.priceValue
        if (price.isNaN() || price <= 0) return "Price must be greater than zero"
        val currency = request.currency.trim()
        if (currency.length != 3) return "Currency must be ISO-4217 code"
        if (request.imageUrls.none { it.isNotBlank() }) return "At least one imageUrl is required"
        val url = request.source.url.trim()
        if (url.isEmpty()) return "Source URL is required"
        return null
    }
